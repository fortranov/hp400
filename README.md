...
| Скрипт | Описание | Подключение |
|--------|----------|-------------|
| `hp_scanner_counter_auto.py` | **Рекомендуемый** - автоматически определяет тип подключения | USB + Сеть |
| `hp_scanner_counter_usb.py` | Для USB подключения | USB |
| `hp_scanner_counter.py` | Для сетевого подключения | Ethernet/Wi-Fi |
| `test_connection_usb.py` | Диагностика USB | - |
| `test_connection.py` | Диагностика сети | - |

## 💡 Использование

### 🌟 Рекомендуемый способ (авто-определение)
```bash
# Автоматически найти и подключиться к принтеру
python hp_scanner_counter_auto.py --scan
python hp_scanner_counter_auto.py --get
python hp_scanner_counter_auto.py --set 1000

# Принудительно использовать сеть
python hp_scanner_counter_auto.py --ip 192.168.1.100 --get
```
